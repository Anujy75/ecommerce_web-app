...
const LocationButton = ({ onDetect, detecting }) => (
  <button
    ...
    onClick={onDetect}
    disabled={detecting}
    style={{ ...S.locBtn, ...(detecting ? S.locBtnActive : {}) }}
    title="Autofill address from current location"
  >
    {detecting ? (
      <><span style={S.locSpinner} />Detecting location…</>
    ) : (
      <>
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(id)) {
      ...
      s.textContent = `
        ...
        @media(max-width:960px){ .co-layout { grid-template-columns: 1fr !important; } }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        switch (error.code) {
          ...
          case error.POSITION_UNAVAILABLE: toast.error("Location unavailable. Try again."); break;
          case error.TIMEOUT:              toast.error("Location request timed out."); break;
          default:                         toast.error("Failed to detect location.");
        }
      },
  ...
  const applyCoupon = () => {
    if (coupon.toUpperCase() === "ANUJ100") {
      setDiscount(100); toast.success("Coupon Applied — ₹100 off!");
    } else if (coupon.toUpperCase() === "MEGA500") {
      setDiscount(500); toast.success("Mega Discount Applied — ₹500 off!");
    } else {
      setDiscount(0); toast.error("Invalid coupon code.");
    }
  };
      ...
      <div style={S.wrapper}>
        ...
        <div style={S.layout} className="co-layout">
          ...
          <form onSubmit={handleSubmit} style={S.formCard}>
            ...
            {/* Shipping */}
            <div style={S.section}>
              <div style={S.sectionHead}><span style={S.sectionDot} />Shipping Details</div>
              ...
              <div style={{ position: "relative" }}>
                <textarea
                  ...
                  required
                />
                {locationDetected && formData.address && <span style={S.fieldCheck}>✓</span>}
              </div>
            </div>

            {/* Payment */}
            <div style={S.section}>
              <div style={S.sectionHead}><span style={S.sectionDot} />Payment Method</div>
            ...
            {/* Coupon */}
            <div style={S.section}>
              <div style={S.sectionHead}><span style={S.sectionDot} />Promo Code</div>
              ...
              {discount > 0 && (
                <div style={S.couponSuccess}>🎉 ₹{discount} discount applied!</div>
              )}
          ...
          <div style={S.summaryCard}>
            <div style={S.summaryHeader}>
              ...
              <div style={S.deliveryTag}>🚀 Delivery by {deliveryDate}</div>
            </div>
...
/* ─── Styles ─────────────────────────────────── */
const S = {
  page:        { minHeight:"100vh", background:"#f5f4f0", padding:"36px 20px 60px", fontFamily:"'Sora','Segoe UI',sans-serif" },
  wrapper:     { maxWidth:1260, margin:"0 auto" },
  header:      { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:28, flexWrap:"wrap", gap:14 },
  heading:     { fontSize:36, fontWeight:800, letterSpacing:"-1.2px", color:"#1c1b18", marginBottom:4 },
  subheading:  { fontSize:14, color:"#9e9b94", fontWeight:500 },
  sslBadge:    { display:"flex", alignItems:"center", gap:7, background:"#fff", border:"1px solid #e4e0d8", borderRadius:100, padding:"8px 18px", fontSize:13, fontWeight:600, color:"#3d3a34" },
  layout:      { display:"grid", gridTemplateColumns:"1.3fr 0.8fr", gap:20, alignItems:"start" },
  formCard:    { background:"#fff", border:"1px solid #e8e4dc", borderRadius:20, padding:"28px 28px 24px", boxShadow:"0 2px 16px rgba(0,0,0,0.05)" },
  section:     { marginBottom:26 },
  sectionHead: { display:"flex", alignItems:"center", gap:8, fontSize:16, fontWeight:700, color:"#1c1b18", marginBottom:16 },
  sectionDot:  { width:8, height:8, borderRadius:"50%", background:"#2d6af6", flexShrink:0, display:"inline-block" },
  locRow:      { display:"flex", alignItems:"center", gap:10, marginBottom:16, flexWrap:"wrap" },
  locBtn:      { display:"inline-flex", alignItems:"center", gap:7, padding:"9px 16px", borderRadius:10, border:"1.5px solid #2d6af6", background:"#eff6ff", color:"#2d6af6", fontSize:13, fontWeight:600, cursor:"pointer", transition:"all 0.2s" },
  locBtnActive:{ background:"#dbeafe", color:"#1d4ed8", animation:"copulse 1.2s ease infinite" },
  locSpinner:  { display:"inline-block", width:12, height:12, border:"2px solid #93c5fd", borderTopColor:"#2d6af6", borderRadius:"50%", animation:"cospin 0.8s linear infinite", flexShrink:0 },
  locBadge:    { display:"inline-flex", alignItems:"center", gap:5, padding:"5px 12px", borderRadius:100, background:"#f0faf4", border:"1px solid #b6e8c8", color:"#1a9c4d", fontSize:12, fontWeight:600, animation:"coslide 0.3s ease" },
  grid2:       { display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 },
  input:       { width:"100%", padding:"13px 16px", borderRadius:12, fontSize:14, color:"#1c1b18", background:"#fafaf8", outline:"none", boxSizing:"border-box", transition:"border 0.2s,background 0.2s", fontFamily:"inherit" },
  textarea:    { width:"100%", minHeight:90, borderRadius:12, padding:"13px 16px", fontSize:14, color:"#1c1b18", background:"#fafaf8", outline:"none", resize:"vertical", boxSizing:"border-box", fontFamily:"inherit", transition:"border 0.2s" },
  fieldCheck:  { position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", color:"#1a9c4d", fontWeight:700, fontSize:14, pointerEvents:"none" },
  payGrid:     { display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 },
  payCard:     { padding:"14px 10px", borderRadius:14, border:"1.5px solid #e4e0d8", background:"#fafaf8", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:5, transition:"all 0.2s" },
  payCardActive:{ border:"1.5px solid #2d6af6", background:"#eff6ff", boxShadow:"0 0 0 3px rgba(45,106,246,0.1)" },
  payIcon:     { fontSize:22 },
  payLabel:    { fontSize:12, fontWeight:700 },
  paySub:      { fontSize:10, color:"#9e9b94" },
  couponRow:   { display:"flex", gap:10, marginBottom:10 },
  couponInput: { flex:1, padding:"12px 14px", borderRadius:12, fontSize:14, color:"#1c1b18", background:"#fafaf8", outline:"none", fontFamily:"inherit", transition:"border 0.2s" },
  applyBtn:    { padding:"0 20px", borderRadius:12, border:"none", background:"#1c1b18", color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", fontFamily:"inherit", letterSpacing:"-0.2px" },
  couponSuccess:{ padding:"9px 14px", borderRadius:10, background:"#f0faf4", border:"1px solid #b6e8c8", color:"#1a9c4d", fontSize:13, fontWeight:600, marginBottom:10, animation:"coslide 0.3s ease" },
  rewardPill:  { padding:"9px 14px", borderRadius:10, background:"#fffbeb", border:"1px solid #fde68a", color:"#92400e", fontSize:13, fontWeight:600 },
  submitBtn:   { width:"100%", padding:"15px", border:"none", borderRadius:14, background:"#1c1b18", color:"#fff", fontSize:16, fontWeight:700, cursor:"pointer", fontFamily:"inherit", letterSpacing:"-0.3px", display:"flex", alignItems:"center", justifyContent:"center", gap:8, transition:"opacity 0.2s,transform 0.1s" },
  btnSpinner:  { width:16, height:16, border:"2px solid rgba(255,255,255,0.3)", borderTopColor:"#fff", borderRadius:"50%", display:"inline-block", animation:"cospin 0.8s linear infinite" },
  summaryCard: { background:"#fff", border:"1px solid #e8e4dc", borderRadius:20, padding:"24px", boxShadow:"0 2px 16px rgba(0,0,0,0.05)", position:"sticky", top:20, height:"fit-content" },
  summaryHeader:{ marginBottom:18 },
  summaryTitle:{ fontSize:20, fontWeight:800, color:"#1c1b18", marginBottom:6, letterSpacing:"-0.5px" },
  deliveryTag: { display:"inline-block", background:"#f0faf4", border:"1px solid #b6e8c8", borderRadius:100, padding:"4px 12px", fontSize:12, fontWeight:600, color:"#1a9c4d" },
  cartList:    { display:"flex", flexDirection:"column", gap:8, marginBottom:18 },
  cartItem:    { display:"flex", alignItems:"center", gap:10, padding:"10px 12px", background:"#fafaf8", border:"1px solid #eeece6", borderRadius:12 },
  cartItemIcon:{ fontSize:20, flexShrink:0 },
  cartItemInfo:{ flex:1, minWidth:0 },
  cartItemName:{ fontSize:13, fontWeight:600, color:"#1c1b18", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" },
  ...
  billRow:     { display:"flex", justifyContent:"space-between", padding:"5px 0", fontSize:13 },
  billLabel:   { color:"#6e6b63" },
  billVal:     { fontWeight:500 },
  totalRow:    { display:"flex", justifyContent:"space-between", alignItems:"center", borderTop:"1.5px solid #1c1b18", paddingTop:14, marginTop:8, fontSize:16, fontWeight:800, color:"#1c1b18" },
  totalAmount: { fontSize:20, letterSpacing:"-0.5px" },
  trustGrid:   { display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginTop:18 },
  trustItem:   { display:"flex", alignItems:"center", gap:7, padding:"8px 10px", background:"#fafaf8", border:"1px solid #eeece6", borderRadius:10, fontSize:12, fontWeight:500, color:"#3d3a34" },
  trustIcon:   { fontSize:15 },
  loaderPage:  { minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#f5f4f0", gap:16, fontFamily:"'Sora',sans-serif" },
  loaderRing:  { width:44, height:44, border:"3px solid #e4e0d8", borderTopColor:"#2d6af6", borderRadius:"50%", animation:"cospin 0.9s linear infinite" },
  loaderText:  { fontSize:14, color:"#9e9b94", fontWeight:500 },
  emptyIcon:   { fontSize:48 },
  emptyTitle:  { fontSize:20, fontWeight:700, color:"#1c1b18" },
  shopBtn:     { padding:"12px 24px", borderRadius:12, border:"none", background:"#1c1b18", color:"#fff", fontWeight:700, fontSize:15, cursor:"pointer", fontFamily:"'Sora',sans-serif" },
};
